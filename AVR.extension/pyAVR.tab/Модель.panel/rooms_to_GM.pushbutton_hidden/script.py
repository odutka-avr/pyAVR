...
if selection:
    ...
    fam_template_path = forms.pick_file(file_ext="rft",
                                            init_dir="C:\ProgramData\Autodesk\RVT " + HOST_APP.version + "\Family Templates\English")
    ...
    for room in selection:
        ...
        catRoom = room.get_Parameter(Guid('912cba47-2e33-4640-88c4-3e7f425ecf99')).AsString()
        if not catRoom:
            catRoom = "Department"
        if not dept:
            dept = "Department"
        ...
        with revit.Transaction("Load Family", doc):
            ...
            set_param = new_fam_instance.get_Parameter(
                DB.BuiltInParameter.ALL_MODEL_INSTANCE_COMMENTS).Set(catRoom)
