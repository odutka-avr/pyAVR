# coding: utf8
##################################################
## Python script in pyRevit
##################################################
## Author: Vladyslav Mashchenko
## Copyright: Copyright 2023
## Credits: [Vladyslav Mashchenko]
## Version: 1.0.0
## Email: vladyslav.mashchenko@outlook.com
##################################################

__doc__ = """Publish Files"""


import clr
import sys
import os
from collections import namedtuple
from Autodesk.Revit.DB.Architecture import Room
from Autodesk.Revit.DB import *
from Autodesk.Revit.DB import Document

import rpw
from rpw import doc, uidoc, DB, UI, db, ui
from rpw.ui.forms import (FlexForm, Label, ComboBox, TextBox, TextBox, Separator, Button, CheckBox)
from pyrevit import forms , revit
from pyrevit import script
output = script.get_output()

clr.AddReference('Microsoft.Office.Interop.Excel')
from Microsoft.Office.Interop import Excel

clr.AddReference("System.Windows.Forms")
from System.Windows.Forms import OpenFileDialog

from pyrevit.compat import safe_strtype

def columnValue(number):
    arry =  xlsheet.Columns[number].Value()
    return arry


fileDialog = OpenFileDialog()
fileDialog.InitialDirectory = "D:\\Work Data\\01_Projects\\AVR.Plagin\\Component"

resolt = str(fileDialog.ShowDialog())
if resolt ==  'OK':
    path = fileDialog.FileName
    xlApp = Excel.ApplicationClass()
    xlApp.Visible = True
    xlbook = xlApp.Workbooks.Open(path)
    xlsheet = xlbook.ActiveSheet

    Foolrstype = columnValue(1)
    Rooms = columnValue(2)
    Levels = columnValue(3)

    xlApp.Quit()



    foolrstype = []
    roomsName = []
    levels = []

    data = []
    for flr, rom ,lvl in zip(Foolrstype, Rooms, Levels):
        if flr == None:
            break
        foolrstype.append(flr)
        roomsName.append(rom.split(","))
        levels.append(lvl)
        data.append([ flr, rom ,lvl ])
    
    output.print_table(table_data=data, 
                        title="Rename type", 
                        columns=[data[0][0],data[0][1], data[0][2]],
                        formats=['', '', '', ])

    print("_______________________________________________")
    foolrstype = foolrstype[1:]
    roomsName =  roomsName[1:]
    levels = levels[1:]

    
    #selected_rooms = []
    def getRoomByName(name):
        #level_filter = ElementLevelFilter(level.Id)  .WherePasses(level_filter)
        room_filter = ElementCategoryFilter(BuiltInCategory.OST_Rooms)
        Rooms = FilteredElementCollector(doc).WhereElementIsNotElementType().WherePasses(room_filter).ToElements()
        roomsAll = []
        for room in Rooms:
            name1 = room.get_Parameter(BuiltInParameter.ROOM_NAME).AsString()
            if name1 == name:
                roomsAll.append(room)
        return roomsAll


    def getFloorTypeByName(name):
        floor_filter = ElementCategoryFilter(BuiltInCategory.OST_Floors)
        FloorsType = FilteredElementCollector(doc).WhereElementIsElementType().WherePasses(floor_filter).ToElements() 
        for floor in FloorsType:
            name1 = floor.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
            if name1 == name:
                return floor

    FloorTypes = []
    Rooms = []
    for rms, floortype in zip( roomsName, foolrstype):
        if len(rms) > 1:
            rooms = []
            for rm in rms:
                room = getRoomByName(rm)
                for i in room:
                    rooms.append(i)
            Rooms.extend([rooms])
            flrtype = getFloorTypeByName(floortype)
            if flrtype != None:
                FloorTypes.append(flrtype)
        else:
            rooms = getRoomByName(rms[0])
            Rooms.append(rooms)
            flrtype = getFloorTypeByName(floortype)
            if flrtype != None:
                FloorTypes.append(flrtype)


    T = Transaction(doc, "Create floors")
    T.Start()
    #############################################################
    ###Get floor_types
    #floor_types = rpw.db.Collector(of_category='OST_Floors', is_type=True).get_elements(wrapped=False)
    ##Select floor type
    #floor_type_options = {DB.Element.Name.GetValue(t): t for t in floor_types}
    ##Select floor types UI
    ##
    #floor_type_id = ""

    components = [#Label('Виберіть тип оздоблення:'),
                  #ComboBox('fl_type', floor_type_options),
                  #Label("Введіть прив`язку підлоги до рівня :"),
                  #TextBox('h_offset', base_offset="50.0"),
                  #CheckBox('checkbox1', 'Брати зміщення підлоги з параметрів приміщення'),
                  CheckBox('checkbox2', 'Вирізати отвори'),
                  CheckBox('checkbox3', 'Вирізати пороги Вікон'),
                  #Label('Вырезание отверстий работает корректно'),
                  #Label('при выборе только одного помещения'),
                  Button('Select')]
    form = FlexForm('Створити оздоблення підлоги', components)
    win = form.show()

    if win == False:
        sys.exit()

    ##Get the ID of floor type
    #if form.values['h_offset'] != "":
    #    offset3 = float(form.values['h_offset'])
    #else:
    #    offset3 = 0

    f ,fls = [], []
    co = []


    for floor_type, selected_rooms in zip(FloorTypes, Rooms):


        def make_floor(new_floor):
            floor_curves = DB.CurveArray()
            for boundary_segment in new_floor.boundary:
                floor_curves.Append((boundary_segment).GetCurve())
            floorType = doc.GetElement(new_floor.floor_type_id)
            level = doc.GetElement(new_floor.level_id)
            normal_plane = DB.XYZ.BasisZ
            f = doc.Create.NewFloor(floor_curves, floorType, level, False, normal_plane)
            fls.append(f)

            # Input parameter values from rooms
            #if form.values['checkbox1'] == True :
            db.Element(f).parameters.builtins['FLOOR_HEIGHTABOVELEVEL_PARAM'].value = float(new_floor.room_offset1)
            #else:
            #    db.Element(f).parameters.builtins['FLOOR_HEIGHTABOVELEVEL_PARAM'].value = float(offset3/304.8)


            #try:
            #    db.Element(f).parameters['BA_AI_RoomName'].value = new_floor.room_name
            #    db.Element(f).parameters['BA_AI_RoomNumber'].value = new_floor.room_number
            #    db.Element(f).parameters['BA_AI_RoomID'].value = new_floor.room_id
            #    db.Element(f).parameters['BA_AI_FinishingType'].value = "Floor Finishing"
            #    db.Element(room).parameters['BA_AI_RoomID'].value = room.Id
            #except:
            #    forms.toaster.send_toast('You need to add shared parameters for BA finishing')
            #    #forms.alert('You need to add shared parameters for BA finishing')
            #    pass


        def make_opening(new_floor):
            co_curves = DB.CurveArray()
            for bounds in i:
                co_curves.Append(bounds.GetCurve())
            co = doc.Create.NewOpening(fls[new_floor.count-1], co_curves, False)



        NewFloor = namedtuple('NewFloor', ['floor_type_id', 'boundary', 'level_id', 'count', 'opening_boundary', 'openings',
                                    'room_offset1', 'room_name', 'room_number', 'room_id'])
    
        new_floors = []
        room_boundary_options = DB.SpatialElementBoundaryOptions()
        opening_boundary = []
        all_boundaries = []
        all_boundaries1 = []
        r = 0


        #print("________________")
        #print(floor_type.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString())

        for room in selected_rooms:

            #print(room.get_Parameter(BuiltInParameter.ROOM_NAME).AsString())

            room_level_id = room.Level.Id
            room_offset1 = room.get_Parameter(BuiltInParameter.ROOM_LOWER_OFFSET).AsDouble()
            room_name = room.get_Parameter(BuiltInParameter.ROOM_NAME).AsString()
            room_number = room.get_Parameter(BuiltInParameter.ROOM_NUMBER).AsString()
            room_id = room.Id
            
            # List of Boundary Segment comes in an array by itself.
            r += 1

            #opening_boundary.append( opening_boundary = room.GetBoundarySegments(room_boundary_options)[1])

            #print( room.GetBoundarySegments(room_boundary_options)[1])
            room_boundary = room.GetBoundarySegments(room_boundary_options)[0]
            all_boundaries = room.GetBoundarySegments(room_boundary_options)

            n = 0

            #
            if len(all_boundaries)>1:
                #for bound_set in all_boundaries:
                #    all_boundaries1.append(room.GetBoundarySegments(room_boundary_options)[n])
                
                all_boundaries1.append(all_boundaries[1])
                n += 1
            else:
                all_boundaries1.append(None)
            all_boundaries1.pop(0)
            new_floor = NewFloor(floor_type_id = floor_type.Id, boundary = room_boundary,
                                 level_id = room_level_id, count = r, opening_boundary = all_boundaries1,
                                 openings = n, room_offset1 = room_offset1, room_name = room_name,
                                 room_number = room_number, room_id = room_id)
            new_floors.append(new_floor)

        c = 0
        #Create floor
        for new_floor in new_floors:
            #with db.Transaction('Create Floor'):
            make_floor(new_floor)
            c += 1
            print(new_floor.openings)
            if form.values['checkbox2'] and new_floor.openings != 0 :
                for i in new_floor.opening_boundary:
                    #with db.Transaction('Create Openeing'):
                    try:
                        make_opening(new_floor)
                    except:
                        continue

    T.Commit()